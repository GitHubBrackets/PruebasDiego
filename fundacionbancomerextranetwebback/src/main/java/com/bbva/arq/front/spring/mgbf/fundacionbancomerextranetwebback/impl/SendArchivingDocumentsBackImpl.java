 ...
 * Contains the implementations of the method sendArchivingDocumentsArmed to
 * consume the service (Send Document-Archiving)
 * 
 * @author Diego Espinoza
 * @version 28 november 2017
 */
public class SendArchivingDocumentsBackImpl implements
SendArchivingDocumentsBack {
    private static final Logger LOG = Logger
    .getLogger(SendArchivingDocumentsBackImpl.class);
    private boolean gtCad = false;
    public static final int VAL = 200;

    /**
     * 
     * Method sendArchivingDocumentsArmed. Contains the parameters for get the
     ...
     *            beanSendArchivingDocumentsIn
     * @param HttpServletRequest
     *            request
     * 
     */
    @Override
    public ResponseEntity<?> sendArchivingDocumentsArmed(
            SendArchivingDocumentsInDTO beanSendArchivingDocumentsIn,
            HttpServletRequest request) {
        LOG.info("Inside sendArchivingDocumentsArmed");
        ...
        try {
            ...
            initialSendArchivingDocumentsServiceImpl = (InitialSendArchivingDocumentsServiceImpl) context
            .getBean("sendArchDocuServiceCli");
            ...
            response = initialSendArchivingDocumentsServiceImpl
            .sendArchivingDocumentsClient(beanSendArchivingDocumentsIn,
                    headers);
            if (response.getStatusCode().value() != VAL) {
                ...
                if (messageErrorDTO.getErrorcode().equals("68")) {
                    ...
                    response = this.sendArchivingDocumentsArmed(
                            beanSendArchivingDocumentsIn, request);
                    gtCad = true;
                }
                if (!gtCad) {
